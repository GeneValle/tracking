<x-action-section>
    <x-slot name="title">
        {{ __('Autentificación de 2 factores') }}
    </x-slot>

    <x-slot name="description">
        {{ __('Añade seguridad adicional para tu cuenta usando la autentiicación de 2 factores') }}
    </x-slot>
        ...
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('Termine de habilitar la autenticación de 2 factores') }}
                @else
                    {{ __('Ha habilitado la autenticación de 2 factores') }}
                @endif
            @else
                {{ __('No has habilitado la autenticación de 2 factores.') }}
            @endif
        </h3>
